...
/*
 * This function splits the sentence into an array containing the words of the sentence
 */
int makeItArray(char line[MAX_LINE_SIZE], char **wordArray) {
    int wordIndex = ZERO;
    char *token; /* This will be used with the strtok method */
...
/*
 * This function removes a new line from the end of the line if there is one
 * Changes the word "word" to be the updated word without a new line at the end
 */
 ...
 * Otherwise, it returns the number of the label in the labels array
 */
int checkLabelName(char word[], label labels[], int labelCount) {
    ...
    /* Compares the given label name to all saved words to ensure it isn't identical */
    for (savedWordIndex = ZERO; savedWordIndex < SAVED_WORDS_AMOUNT; savedWordIndex++) {
        if (strcmp(word, savedWord[savedWordIndex]) == ZERO) {
            return MINUS_TWO;
        }
    }
    /* Compares the given label name to all previous labels names to ensure it isn't identical to any of the previous labels */
...
/*
 * This function calculates and sets the base and offset for labels
 */
...
int countCommas(char line[MAX_LINE_SIZE]) {
    int ind;
    int count = ZERO;
    /* Counts how many commas there are and returns the count */
...
int makeItNumArray(int IntArray[MAX_LINE_SIZE], char intLine[MAX_LINE_SIZE]) {
    ...
    /* Check if the current cell is a number without characters - if it is, add it to the array - else return -2 */
    for (intArrayIndex = ZERO; intArrayIndex <= index; intArrayIndex++) {
        tempNum = strtol(tempIntCharLines[intArrayIndex], &endPtr, TEN);

        /* Check if the first char is not end of line or end of string */
        if (*endPtr != '\0' && *endPtr != '\n') {
            return MINUS_TWO;
        }
...
/*
 * This function is used to find the number of the command in the commands array
 */
int searchCommand(command commands[], char *name) {
    ...
    for (i = ZERO; i < AMOUNT_OF_COMMANDS; i++) {
        if ((strcmp(name, commands[i].name)) ==
            ZERO) {   /* The name sent has the same name as one of the commands in the commands array */
            return i;
        }
    }
    return MINUS_ONE;
 ...
 * 2. Split the operands into different cells in the commandOperands array
 *
 * Return -1 if the commas and the operands are not balanced
 * Otherwise, it will return the number of operands;
 */
int lineForCommand(char *wordArray[MAX_LINE_SIZE], int labelFlag, int wordAmount,
    ...
    /*
     * Make the operands one line separated by " " or ","
     */
    ...
    /*
     * Count how many operands the line contains
     */
    numOfOperands = numOfOperandsInLine(line);
    /*
     * Count how many commas the line contains
     */
    ...
    if (numOfCommas + ONE != numOfOperands) {
        /* Error */
        printf("Line %d: The count of commas and operands does not match as expected.\n", lineCounter);
        return MINUS_ONE;
    }
...
/*
 * This function will remove all spaces from a line of characters
 */
...
/*
 * This function counts how many operands are in the line
 * Every operand in the line is separated by " " or ","
 */
int numOfOperandsInLine(char *line) {
    ...
    while (line[index] != '\0') {
        if (operandFlag == ZERO && line[index] != ' ' &&
            line[index] != ',') /* If we reached a ' ' or ',' we will increase the count */
        {
            operandCount++;
...
/* This function prints an error when the number of operands does not match */
...
int findAddressMethod(char *operand, char registerTable[MAX_LINE_SIZE][MAX_LINE_SIZE], int lineCounter) {
    ...
    if (operand[ZERO] == '#') {
        while (operand[numberIndex] != '\0' && operand[numberIndex] != '\n') {
            if (((isdigit(operand[numberIndex])) == ZERO) &&
                operand[numberIndex] != '-')/* Checks if all the characters are digits or '-' */
            {
                printf("Line %d: The operand is invalid.\n", lineCounter);
                return MINUS_ONE;
            }
    ...
    */
    ...
    return ONE; /* Means maybe a label that's defined later */
}


/*
 * This function returns the number of additional words to be added according to the addressing method from the receipt
 */
...
int addLength(int addressingMethod) {
    ...
    /* Returns a value depending on the addressing method */
}

/*
 *The function receives the command name and addressing methods and fills the appropriate memory accordingly
 */
void changeMemoryFirstCross(int firstOperandAddressMethod, int secondOperandAddressMethod, int *RAM, int IC, int commandIndex, command commands[], int numOfOperands, char registerLabel[MAX_LINE_SIZE][MAX_LINE_SIZE], char commandOperands[MAX_LINE_SIZE][MAX_LINE_SIZE]) {
    ...
    RAM[IC] |= (ONE << commands[commandIndex].opcode);       /* Inserts the opcode into the first cell in the label memory */
    RAM[IC] |= (ONE << INT_A);  /* Turns on the bit in the place of A (the 18 bit) */

    /*
    * If numOfOperands equals ZERO then there is only one line, no need to create another line for the command
     */
    if (numOfOperands != ZERO) {

        IC++;   /* Move to the next line */
        RAM[IC] |= (commands[commandIndex].funct << FUNCT);    /* Inserts the funct into its cell in the label memory */
        RAM[IC] |= (ONE << INT_A);  /* Turns on the bit in the place of A (the 18 bit) */
    }
    ...
    if (numOfOperands == ONE) {
        ...
        RAM[IC] |= (firstOperandAddressMethod);    /* Inserts the target addressing method into its cell in the label memory */
        ...
        if (firstOperandAddressMethod == TWO || firstOperandAddressMethod == THREE) {
            ...
            RAM[IC] |= (registerNumber << TARGET_REGISTER);     /* Inserts the target register into its cell in the label memory */

        } else if (firstOperandAddressMethod == ZERO) {
            IC++;
            RAM[IC] |= getNumAddressMethod1(commandOperands[ZERO]); /* Inserts the target addressing method into its cell in the label memory */
            RAM[IC] &= SIXTEEN_BITS_ON;    /* Turns off bits higher than 16 */
            RAM[IC] |= (ONE << INT_A);  /* Turns on the bit in the place of A (the 18 bit) */
        }
    ...
    /*
     * If the method gets 2 operands
     */
    if (numOfOperands == TWO) {
        /*
         * Check if the first Line needs to be entered
         */
        {
            /*Insert ORIGIN_ADDRESS*/
            RAM[IC] |= (firstOperandAddressMethod << ORIGIN_ADDRESS);

            /*Check if ORIGIN register needs to be changed*/
            if (firstOperandAddressMethod == TWO || firstOperandAddressMethod == THREE) {
                registerNumber = findRegister(commandOperands[ZERO], registerLabel, firstOperandAddressMethod);
                RAM[IC] |= (registerNumber << ORIGIN_REGISTER); /* Inserts the origin register into its cell in the label memory */
            }
        ...
        if (firstOperandAddressMethod == ZERO) {
            RAM[IC] |= getNumAddressMethod1(
                    commandOperands[ZERO]); /* Inserts the target addressing method into its cell in the label memory */
            RAM[IC] &= SIXTEEN_BITS_ON; /* Turns off bits higher than 16 */
            RAM[IC] |= (ONE << INT_A); /* Turns on the bit in the place of A (the 18 bit) */
            IC++;
        }
        ...
        */
        ...
        if (secondOperandAddressMethod == ZERO) {
            RAM[IC] |= getNumAddressMethod1(
                    commandOperands[ONE]); /* Inserts the target addressing method into its cell in the label memory */
            RAM[IC] &= SIXTEEN_BITS_ON; /* Turns off bits higher than 16 */
            RAM[IC] |= (ONE << INT_A); /* Turns on the bit in the place of A (the 18 bit) */
        }
    }
...
/*
 * This function converts a string containing the register name to an int representing the register index number in the array of registers
 */
int
...
/*
 * This function gets a string and converts it to an int, works for positive and negative numbers
 * The function ignores the first char which is supposed to be '#'
 */
...
/*
 *  This function finds the label number in the table
 */
...
/*
 * This function goes through all the chars in a string and check if they are equal to ']'
 */
...
/*
 * This function removes a line break at the end of the incoming output
 */
